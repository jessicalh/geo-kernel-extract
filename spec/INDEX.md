...
- **WIP_OBJECT_MODEL.md** — WORK IN PROGRESS, supersedes portions of OBJECT_MODEL.md and PATTERNS.md until folded in. Design for the trajectory-scope object model: TrajectoryProtein (replaces GromacsProtein role), TrajectoryAtom (replaces GromacsProteinAtom content), TrajectoryResult (modular calculator at trajectory scope, parallel to ConformationResult), Trajectory (process entity with Run method), RunConfiguration (three named configs: ScanForDftPointSet, PerFrameExtractionSet, FullFatFrameExtraction), RunContext (class not struct), NmrAtomIdentity (topology-like typed identity on Protein). Bundled with NamingRegistry activation for single coordinated pre-fleet library update. Verbose and concrete with worked examples (BsWelford, BsT0Autocorrelation), plus Appendices A-H closing specific gaps surfaced by a 2026-04-22 pm cold-read review. **§0 carries a non-regression contract — static PDB/ORCA paths must continue to work.** §2 NmrAtomIdentity, §3 TrajectoryBond subsection, and Appendices A / B / E are flagged PROPOSAL-PENDING-USER-REVIEW; those don't reach implementation until user sign-off. Header notes in OBJECT_MODEL.md and PATTERNS.md point here during the design window.
