...
void CsaTensorOverlay::ensureActors() {
    ...
    mapper->SetInputConnection(glyphFilter_->GetOutputPort());
    ...
    glyphActor_->GetProperty()->SetAmbient(0.35);
    glyphActor_->GetProperty()->SetDiffuse(0.80);
    glyphActor_->GetProperty()->SetSpecular(0.10);
    glyphActor_->GetProperty()->SetSpecularPower(15);
...
void CsaTensorOverlay::show(const model::Vec3& atomPos,
    ...
    // |dev|-based radii: the shape is symmetric in size (Schultz-Kindlmann); the
    // SIGN of each direction is carried by colour, not by length.
    std::array<double, 3> radii{};
    for (int i = 0; i < 3; ++i)
        radii[i] = kBaseRadius * (kFloorFrac + (1.0 - kFloorFrac) * std::abs(dev[i]) / maxAbs);

    // Kindlmann linear/planar anisotropy on |dev| (sorted hi >= mid >= lo) drives
    // the superquadric edge sharpness: linear -> cigar along the largest-|dev|
    // axis; planar -> disc whose normal is the smallest-|dev| axis; isotropic ->
    // sphere. This is the shape disambiguation a plain ellipsoid cannot give.
    ...
    // Deep-copy the unit superquadric and attach the per-vertex sign scalar (the
    // signed directional deviation n^T diag(dev) n / maxAbs in [-1,1]). Local
    // axes map X->axisA, Y->uniqueAxis (the vtk superquadric symmetry axis),
    // Z->axisB; the colour is direction-based so it is correct regardless.
    ...
    // Transform: local X/Y/Z -> (axisA, uniqueAxis, axisB) eigenvectors scaled by
    // their radii; translate to the atom.
    ...
    for (int axis = 0; axis < 3; ++axis) {
        ...
        const model::Vec3 tip = atomPos + d * (radii[static_cast<std::size_t>(axis)] + kLabelGap);
