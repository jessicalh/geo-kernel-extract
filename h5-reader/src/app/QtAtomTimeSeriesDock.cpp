...
const std::vector<Desc>& descs() {
    static const std::vector<Desc> kDescs = {
        ...
        {"Bond anisotropy",
         "mc_shielding |T2|",
         "Å⁻³",
         [](const QtFrame& f, std::size_t a) { return T2Mag(f.mcShielding(a)); }},
        ...
        // Electrostatics
        {"Electrostatics",
         "coulomb_shielding T0",
         "ppm",
         [](const QtFrame& f, std::size_t a) { return f.coulombShielding(a).T0; }},
        {"Electrostatics", "|E| Coulomb", "V/Å", [](const QtFrame& f, std::size_t a) { return f.coulombETotal(a).norm(); }},
        {"Electrostatics",
         "|E| magnitude field",
         "V/Å",
         [](const QtFrame& f, std::size_t a) { return f.coulombEMagnitude(a); }},
        ...
        {"Electrostatics",
         "AIMNet2 shielding T0",
         "ppm",
         [](const QtFrame& f, std::size_t a) { return f.aimnet2Shielding(a).T0; }},
...
int descCount() {
    return static_cast<int>(descs().size());
}

QString comboLabel(const Desc& d) {
    QString s = QStringLiteral("%1: %2").arg(QString::fromLatin1(d.group), QString::fromLatin1(d.label));
...
QtAtomTimeSeriesDock::QtAtomTimeSeriesDock(QWidget* parent) : QDockWidget(QStringLiteral("Time Series"), parent) {
...
void QtAtomTimeSeriesDock::rebuildSeries() {
    if (!dataSeries_ || !yAxis_ || !xAxis_)
        return;
    if (!protein_ || !conformation_ || !hasSelection_)
        return;
    ...
    chart_->setTitle(QStringLiteral("%1  —  atom %2").arg(QString::fromLatin1(d.label)).arg(atomIdx_));

    qCDebug(cTs).noquote() << "rebuild | atom=" << atomIdx_ << "| scalar=" << d.label << "| n=" << pts.size() << "| y=[" << yMin
                           << "," << yMax << "]";
}

void QtAtomTimeSeriesDock::updateCursor() {
    if (!cursorSeries_ || !yAxis_)
        return;
