...
FrameVariant classifyFrameVariant(const model::QtProtein& p, std::size_t atomIdx) {
    ...
    const model::QtResidue* r = hasResidue ? &p.residue(static_cast<std::size_t>(a.residueIndex)) : nullptr;
    ...
    if (a.IsBackboneAlphaCarbon())
        return FrameVariant::BackboneCA;
    if (a.IsBackboneCarbonylCarbon())
        return FrameVariant::BackboneCarbonylC;
    if (a.IsBackboneCarbonylOxygen())
        return FrameVariant::BackboneCarbonylO;
    if (a.IsBackboneAmideHydrogen())
        return hasPrev ? FrameVariant::HN_Standard : FrameVariant::HN_NTerminus;
    if (a.IsAnyAlphaHydrogen())
        return FrameVariant::BackboneHA;
...
Relationship makeAimnet2FeatureRelationship() {
    Relationship rel;
    rel.name = QStringLiteral("aimnet2_features");
...
Aimnet2FeatureStats RunAimnet2PerAtomFeature(const Body& body, Aimnet2FeatureSink& sink) {
    ...
    const model::QtProtein& p = *body.run.protein;
    qCInfo(cAimnet2).noquote() << "aimnet2_features per_atom_feature | atoms=" << p.atomCount()
    ...
    const Relationship rel = makeAimnet2FeatureRelationship();
    RunTraversal(
        rel,
        body,
        [&](std::size_t atom, std::size_t row, std::size_t orig, const FrameResult& fr, const NeighborhoodRecord& rec) {
            ...
            out.frame_variant = fr.frame.is_valid ? static_cast<int>(fr.frame.variant)
            ...
            if (target.local_frame_valid)
                out.dft_target_local_T2 = DecomposeLibrary(target.total_local).T2;
            ...
            if (out.aimnet2_charge_present) {
                ...
                if (std::isfinite(out.aimnet2_charge))
                    ++stats.charge_present;
            }
            ...
            if (out.crg_present) {
                ...
                if (out.frame_valid)
                    out.crg_vector_local = fr.frame.ToLocal(out.crg_vector_lab);
            ...
            if (out.embedding_present && embeddingDims == sink.embeddingDims())
                ++stats.embedding_present;
    ...
    qCInfo(cAimnet2).noquote() << "aimnet2_features per_atom_feature rows=" << stats.rows
                               << "| dft_present=" << stats.dft_present << "| frame_valid=" << stats.frame_valid
                               << "| charge_present=" << stats.charge_present << "| crg_present=" << stats.crg_present
