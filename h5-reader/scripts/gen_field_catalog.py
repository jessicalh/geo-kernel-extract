...
is the SDK source of truth for producer-owned NPY arrays. This script mirrors
the producer CATALOG into a typed C++ table so the reader resolves a producer
filename stem to a typed FieldSpec exactly once (FindFieldByStem) and then
dispatches on the FieldKind / FieldGroup / NativeAxis enums. Other Reader
signal sources are declared in TrajectorySignalCatalog.
...
def main() -> int:
    ...
    w(f"// from {CATALOG_PY.relative_to(ROOT).as_posix()}")
    ...
    w("// to a typed FieldSpec exactly once (FindFieldByStem). Other Reader signal")
    w("// sources are declared in TrajectorySignalCatalog. wrapper/irreps/units/")
