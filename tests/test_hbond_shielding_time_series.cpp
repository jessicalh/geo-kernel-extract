//
// test_hbond_shielding_time_series: discipline + integration for
// HBondShieldingTimeSeriesTrajectoryResult. HBondResult requires
// DsspResult, so DSSP is not skipped here.
//
...
TEST(HBondShieldingTimeSeries, SyntheticFourFrames) {
    ...
    auto tr = nmr::HBondShieldingTimeSeriesTrajectoryResult::Create(tp);
...
TEST(HBondShieldingTimeSeries, Frame0Semantics) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    opts.skip_dssp = false;  // HBondResult deps on DSSP
    ...
    config.AddTrajectoryResultFactory([](const nmr::TrajectoryProtein& tp_in)
        ...
        return nmr::HBondShieldingTimeSeriesTrajectoryResult::Create(tp_in);
    });
...
TEST(HBondShieldingTimeSeries, FinalizeIdempotency) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    auto* buf_first = tp.GetDenseBuffer<nmr::SphericalTensor>(
        std::type_index(typeid(nmr::HBondShieldingTimeSeriesTrajectoryResult)));
    const std::size_t T_first = buf_first->StridePerAtom();
    auto& tr = tp.Result<nmr::HBondShieldingTimeSeriesTrajectoryResult>();
    tr.Finalize(tp, traj);
...
TEST(HBondShieldingTimeSeries, H5RoundTrip) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
...
TEST(HBondShieldingTimeSeries, Integration1P9J) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    auto* buf = tp.GetDenseBuffer<nmr::SphericalTensor>(std::type_index(
        typeid(nmr::HBondShieldingTimeSeriesTrajectoryResult)));
    ASSERT_NE(buf, nullptr);
    ...
    std::cout << "HBondShieldingTimeSeries populated=" << populated
              << "/" << buf->AtomCount() << "\n";
}
