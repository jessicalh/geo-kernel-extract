//
// test_larsen_hbond_1pHB_shielding_time_series: discipline + integration
// tests for LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult.
//
// Five tests, cloned from test_tripeptide_bb_shielding_time_series.cpp
// against the larsen_hbond_1pHB_spherical source field.
//
...
// Per-class HB shielding sanity ceiling (log-only). Total Larsen HB
// shift on amide H is ~5-10 ppm; per-class is a fraction. 50 ppm is
// a generous log threshold that lets the calculator keep trucking.
...
TEST(LarsenHBond1pHBShieldingTimeSeries,
     FixtureIndependentProductionH5Contract) {
    constexpr std::size_t kAtoms = 2;
    constexpr std::size_t kFrames = 2;

    // Compute on an empty, in-memory conformation records the real
    // production provenance vectors without requiring TPR/TRR/EDR files.
    // The dense buffer is public trajectory storage and is populated with
    // an explicit atom axis solely to drive the production H5 writer.
    nmr::TrajectoryProtein tp;
    auto result =
        nmr::LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::Create(tp);
    ...
    result->Compute(empty_conf, tp, trajectory, 11, 1.25);  // absent
    result->ForceSourcePresentForTesting(true);
    result->Compute(empty_conf, tp, trajectory, 19, 2.5);   // present
    ...
    tp.AdoptDenseBuffer<nmr::SphericalTensor>(
        ...
        std::type_index(typeid(
            nmr::LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult)));

    const std::string h5_path = (fs::temp_directory_path() /
        ("larsen_hbond_1pHB_a22_contract_" +
    ...
    for (std::size_t atom = 0; atom < kAtoms; ++atom) {
        for (std::size_t component = 0; component < 9; ++component) {
            EXPECT_TRUE(std::isnan(flat[(atom * kFrames) * 9 + component]));
        }
        double expected[9] = {};
        SyntheticTensor(atom, 1).PackFull9(expected);
        for (std::size_t component = 0; component < 9; ++component) {
            EXPECT_DOUBLE_EQ(
                flat[(atom * kFrames + 1) * 9 + component],
                expected[component]);
        }
    }
    ...
    const std::uint64_t mask_sum = source_mask[0] + source_mask[1];
    EXPECT_EQ(source_count, mask_sum);
...
TEST(LarsenHBond1pHBShieldingTimeSeries, SyntheticFourFrames) {
    ...
    auto tr = nmr::LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::Create(tp);
    ...
    for (size_t t = 0; t < kFrames; ++t) {
        ...
        for (size_t i = 0; i < Ntp; ++i)
            conf->MutableAtomAt(i).larsen_hbond_1pHB_spherical =
                SyntheticTensor(i, t);
...
TEST(LarsenHBond1pHBShieldingTimeSeries, Frame0Semantics) {
    ...
    config.SetName("LarsenHBond1pHBShieldingTimeSeriesFrame0Semantics");
    ...
    config.AddTrajectoryResultFactory(
        [](const nmr::TrajectoryProtein& tp_in)
            ...
            return nmr::LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult
                    ::Create(tp_in);
        });
    ...
    auto* buf = tp.GetDenseBuffer<nmr::SphericalTensor>(std::type_index(
        typeid(nmr::LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult)));
    ASSERT_NE(buf, nullptr);
...
TEST(LarsenHBond1pHBShieldingTimeSeries, FinalizeIdempotency) {
    ...
    nmr::RunConfiguration config;
    config.SetName("LarsenHBond1pHBShieldingTimeSeriesFinalizeIdempotency");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    opts.skip_apbs = true;  opts.skip_dssp = true;
    config.RequireConformationResult(typeid(nmr::GeometryResult));
...
TEST(LarsenHBond1pHBShieldingTimeSeries, H5RoundTrip) {
    ...
    nmr::RunConfiguration config;
    config.SetName("LarsenHBond1pHBShieldingTimeSeriesH5RoundTrip");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    ...
    const std::string h5_path = (fs::temp_directory_path() /
        ("larsen_hbond_1pHB_ts_h5_roundtrip_" +
         std::to_string(::getpid()) + ".h5")).string();
    {
        HighFive::File file(h5_path, HighFive::File::Truncate);
        tr.WriteH5Group(tp, file);
...
TEST(LarsenHBond1pHBShieldingTimeSeries, IntegrationLogOverages1P9J) {
    ...
    nmr::RunConfiguration config;
    config.SetName("LarsenHBond1pHBShieldingTimeSeriesIntegration");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    ...
    for (std::size_t i = 0; i < N; ++i) {
        for (std::size_t t = 0; t < T; ++t) {
            ...
            if (abs_t0 > max_t0_magnitude) max_t0_magnitude = abs_t0;
            if (abs_t0 > kPerClassT0SanityPpm) ++overage_count;
            if (t == 0 && abs_t0 > 1e-12) ++nonzero_at_frame0;
        }
    }

    // Real-data coverage floor: with the grid loaded, the per-class
    // calc should produce nonzero T0 on a nontrivial fraction of atoms
    // at frame 0. Floor = N/100 catches "every cell stayed at default
    // 0.0" regressions while accommodating that only a subset of atoms
    // contribute to any one H-bond term class.
    const std::size_t coverage_floor = buf->AtomCount() / 100;
    EXPECT_GT(nonzero_at_frame0, coverage_floor)
        << "Larsen 1pHB produced nonzero T0 on only " << nonzero_at_frame0
        << " of " << N << " atoms at frame 0 (floor=" << coverage_floor
        << ") — calc likely never ran despite grid loaded";

    if (overage_count > 0) {
        std::cout << "[warn] LarsenHBond1pHB: " << overage_count
                  << " cells exceed " << kPerClassT0SanityPpm
                  << " ppm (max=" << max_t0_magnitude << ") — Larsen grid "
                  << "extreme bin, not asserted\n";
    }

    std::cout << "LarsenHBond1pHB diagnostic: frames=" << T
              << " atoms=" << N
              << " nonzero_frame0=" << nonzero_at_frame0 << "/" << N
              << " max_T0=" << max_t0_magnitude
