//
// test_disp_shielding_time_series: discipline + integration for
...
TEST(DispersionShieldingTimeSeries, SyntheticFourFrames) {
    ...
    auto tr = nmr::DispersionShieldingTimeSeriesTrajectoryResult::Create(tp);
    ...
    for (size_t t = 0; t < kFrames; ++t) {
        ...
        for (size_t i = 0; i < Ntp; ++i)
            conf->MutableAtomAt(i).disp_shielding_contribution = SyntheticTensor(i, t);
        tr->Compute(*conf, tp, traj, t, static_cast<double>(t));
    }
    tr->Finalize(tp, traj);

    auto* buf = tp.GetDenseBuffer<nmr::SphericalTensor>(std::type_index(typeid(
        nmr::DispersionShieldingTimeSeriesTrajectoryResult)));
    ...
    const std::string h5_path = (fs::temp_directory_path() /
        ("disp_shielding_ts_unit_" + std::to_string(::getpid()) + ".h5")).string();
    { HighFive::File file(h5_path, HighFive::File::Truncate); tr->WriteH5Group(tp, file); }
    HighFive::File reopen(h5_path, HighFive::File::ReadOnly);
    ASSERT_TRUE(reopen.exist("/trajectory/disp_shielding_time_series"));
    fs::remove(h5_path);
}


TEST(DispersionShieldingTimeSeries, Frame0Semantics) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    config.RequireConformationResult(typeid(nmr::DispersionResult));
    config.AddTrajectoryResultFactory([](const nmr::TrajectoryProtein& tp_in)
        -> std::unique_ptr<nmr::TrajectoryResult> {
        return nmr::DispersionShieldingTimeSeriesTrajectoryResult::Create(tp_in);
    });
...
TEST(DispersionShieldingTimeSeries, FinalizeIdempotency) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    auto* buf_first = tp.GetDenseBuffer<nmr::SphericalTensor>(
        std::type_index(typeid(nmr::DispersionShieldingTimeSeriesTrajectoryResult)));
    const std::size_t T_first = buf_first->StridePerAtom();
    auto& tr = tp.Result<nmr::DispersionShieldingTimeSeriesTrajectoryResult>();
    tr.Finalize(tp, traj);
...
TEST(DispersionShieldingTimeSeries, H5RoundTrip) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    const auto& tr = tp.Result<nmr::DispersionShieldingTimeSeriesTrajectoryResult>();
    const std::string h5_path = (fs::temp_directory_path() /
        ("disp_shielding_ts_h5_roundtrip_" + std::to_string(::getpid()) + ".h5")).string();
    { HighFive::File file(h5_path, HighFive::File::Truncate); tr.WriteH5Group(tp, file); }
    HighFive::File reopen(h5_path, HighFive::File::ReadOnly);
    auto grp = reopen.getGroup("/trajectory/disp_shielding_time_series");
...
TEST(DispersionShieldingTimeSeries, Integration1P9J) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";
    ...
    auto* buf = tp.GetDenseBuffer<nmr::SphericalTensor>(std::type_index(
        typeid(nmr::DispersionShieldingTimeSeriesTrajectoryResult)));
    ASSERT_NE(buf, nullptr);

    // Dispersion kernel is pure-T2 (Tr(K) = S(r)*(3r²/r⁸ - 3/r⁶) = 0
    // analytically; see DispersionResult.h:17-19). T0 is structurally
    // zero; the signal lives in T2. Check max |T2[k]|.
    ...
    EXPECT_GT(max_abs_t2, 1e-6) << "Dispersion T2 channels at noise floor — kernel may be broken";
    std::cout << "DispersionShieldingTimeSeries max|T2|=" << max_abs_t2
              << " A^-6, populated_t2=" << populated_t2
              << "/" << buf->AtomCount() << " (pure-T2 traceless; 1/r^6 falloff + 4.3-5.0 A switching)\n";
}
