...
// EnrichmentResult (the TripeptideNeighborShieldingResult deps) plus
// the time-series TR under test. APBS / AIMNet2 / MOPAC / DSSP all
// skipped — they are not on the TripeptideNeighborShieldingResult
// dependency chain. RequireConformationResult includes
// TripeptideNeighborShieldingResult so Phase 4 dep-validation passes;
// OperationRunner will attach it conditionally when
// opts.tripeptide_dft_table is non-null (set in Trajectory::Run Phase
// 5 from Session::TripeptideDftTablePtr).
void ConfigureNeighborTimeSeries(nmr::RunConfiguration& config,
    ...
    config.RequireConformationResult(typeid(nmr::TripeptideNeighborShieldingResult));
...
TEST(TripeptideNeighborShieldingTimeSeries, IntegrationFingerprint1P9J) {
    ...
    // Finite and physically plausible bound. Larsen 2015 cap-side
    // Δσ_BB^{i±1} per-direction magnitudes are in the [0.1, 5] ppm
    // band; the SUM on the central CA is bounded by ~10 ppm.
    //
    // NOT-YET-BAKED FINGERPRINT: a tight (3-sig-fig) fingerprint
    // assertion against a specific T0 value is intentionally NOT
    // landed in this commit because the sandbox under which this
    // change was authored does not allow running the test binary
    // (libssl/libcrypto rpath mismatch on the discovery probe). The
    // first time the test runs cleanly on batcave, replace this
    // EXPECT with EXPECT_NEAR(st.T0, kObserved, 0.005) using the
    // value printed above.
    EXPECT_TRUE(std::isfinite(st.T0)) << "T0 is not finite";
    EXPECT_LT(std::abs(st.T0), 10.0)
        << "T0=" << st.T0 << " ppm exceeds Larsen 2015 cap-side band";
}
