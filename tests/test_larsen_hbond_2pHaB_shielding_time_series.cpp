//
// test_larsen_hbond_2pHaB_shielding_time_series: clone of 1pHB tests
// against larsen_hbond_2pHaB_spherical.
//
...
TEST(LarsenHBond2pHaBShieldingTimeSeries, SyntheticFourFrames) {
    ...
    auto tr = nmr::LarsenHBond2pHaBShieldingTimeSeriesTrajectoryResult::Create(tp);
    ...
    for (size_t t = 0; t < kFrames; ++t) {
        ...
        for (size_t i = 0; i < Ntp; ++i)
            conf->MutableAtomAt(i).larsen_hbond_2pHaB_spherical =
                SyntheticTensor(i, t);
...
TEST(LarsenHBond2pHaBShieldingTimeSeries, Frame0Semantics) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";

    nmr::RunConfiguration config;
    config.SetName("LarsenHBond2pHaBShieldingTimeSeriesFrame0Semantics");
    ...
    config.AddTrajectoryResultFactory(
        [](const nmr::TrajectoryProtein& tp_in)
            ...
            return nmr::LarsenHBond2pHaBShieldingTimeSeriesTrajectoryResult
                    ::Create(tp_in);
        });
    ...
    auto* buf = tp.GetDenseBuffer<nmr::SphericalTensor>(std::type_index(
        typeid(nmr::LarsenHBond2pHaBShieldingTimeSeriesTrajectoryResult)));
    ASSERT_NE(buf, nullptr);
...
TEST(LarsenHBond2pHaBShieldingTimeSeries, FinalizeIdempotency) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";

    nmr::RunConfiguration config;
    config.SetName("LarsenHBond2pHaBShieldingTimeSeriesFinalizeIdempotency");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    opts.skip_apbs = true;  opts.skip_dssp = true;
    config.RequireConformationResult(typeid(nmr::GeometryResult));
...
TEST(LarsenHBond2pHaBShieldingTimeSeries, H5RoundTrip) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    nmr::Session session;
    ...
    nmr::RunConfiguration config;
    config.SetName("LarsenHBond2pHaBShieldingTimeSeriesH5RoundTrip");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    ...
    const std::string h5_path = (fs::temp_directory_path() /
        ("larsen_hbond_2pHaB_ts_h5_roundtrip_" +
         std::to_string(::getpid()) + ".h5")).string();
    {
        HighFive::File file(h5_path, HighFive::File::Truncate);
        tr.WriteH5Group(tp, file);
...
TEST(LarsenHBond2pHaBShieldingTimeSeries, IntegrationLogOverages1P9J) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    ...
    nmr::RunConfiguration config;
    config.SetName("LarsenHBond2pHaBShieldingTimeSeriesIntegration");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac = true; opts.skip_coulomb = true;
    ...
    EXPECT_GT(nonzero_at_frame0, coverage_floor)
        << "Larsen 2pHaB produced nonzero T0 on only " << nonzero_at_frame0
        << " of " << buf->AtomCount() << " atoms at frame 0 (floor="
        << coverage_floor << ") — calc likely never ran despite grid loaded";

    if (overage_count > 0) {
        std::cout << "[warn] LarsenHBond2pHaB: " << overage_count
                  << " cells exceed " << kPerClassT0SanityPpm
                  << " ppm (max=" << max_t0 << ")\n";
    }
    std::cout << "LarsenHBond2pHaB diagnostic: max_T0=" << max_t0
