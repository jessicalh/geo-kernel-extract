...
TEST(HmWelford, H5DirectionalMetadataZeroCountSynthetic) {
    nmr::test::TestEnvironment::LoadCalculatorConfig();
    ...
    auto tr = nmr::HmWelfordTrajectoryResult::Create(tp);
    nmr::Trajectory traj({}, {}, {});
    tr->Compute(tp.CanonicalConformation(), tp, traj, 7, 2.5);
    tp.MutableAtomAt(0).hm_welford = {};
    tr->Finalize(tp, traj);

    const std::string h5_path = (fs::temp_directory_path() /
        ("hm_welford_directional_metadata_" +
    ...
    auto grp = file.getGroup("/trajectory/hm_welford");
...
TEST(HmWelford, H5RoundTrip) {
    ...
    // Keep file at a stable temp location for manual inspection
    const std::string inspect_path =
        nmr::test::TestEnvironment::TempPath("hm_welford_inspect.h5");
    std::cout << "INSPECT_DEBUG: copying " << h5_path << " to " << inspect_path << "\n";
    try {
        fs::copy_file(h5_path, inspect_path,
