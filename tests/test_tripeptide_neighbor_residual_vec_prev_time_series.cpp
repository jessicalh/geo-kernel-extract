//
// test_tripeptide_neighbor_residual_vec_prev_time_series: discipline +
// integration tests for TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult.
//
// Five tests, mirroring the BB residual_vec TR pattern against the
// i-1-direction neighbor residual buffer:
//
//   1. SyntheticFourFrames — finite Vec3 round-trip via Compute /
//      Finalize / WriteH5Group.
//   2. Frame0Semantics, 3. FinalizeIdempotency, 4. H5RoundTrip
//   5. IntegrationLogOverages1P9J — NaN-tolerant: per-direction residual
//      is NaN-initialised by TripeptideNeighborShieldingResult, so NaN
//      cells mean "no i-1 contribution this frame" and are signal.
//      We tally finite vs NaN counts, log overages on finite cells,
//      and floor on finite-and-nonzero population.
//
...
TEST(TripeptideNeighborResidualVecPrevTimeSeries, SyntheticFourFrames) {
    ...
    for (size_t t = 0; t < kFrames; ++t) {
        ...
        for (size_t i = 0; i < Ntp; ++i) {
            conf->MutableAtomAt(i).tripeptide_neighbor_residual_vec_prev =
                SyntheticVec3(i, t);
        }
    ...
    auto* buf = tp.GetDenseBuffer<nmr::Vec3>(std::type_index(typeid(
        nmr::TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult)));
    ...
    const std::string h5_path = (fs::temp_directory_path() /
        ("tripeptide_neighbor_residual_vec_prev_ts_unit_" +
...
TEST(TripeptideNeighborResidualVecPrevTimeSeries, Frame0Semantics) {
    ...
    config.AddTrajectoryResultFactory(
        [](const nmr::TrajectoryProtein& tp_in)
            ...
            return nmr::TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult
                    ::Create(tp_in);
        });
    ...
    auto* buf = tp.GetDenseBuffer<nmr::Vec3>(std::type_index(
        typeid(nmr::TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult)));
    ASSERT_NE(buf, nullptr);
...
TEST(TripeptideNeighborResidualVecPrevTimeSeries, FinalizeIdempotency) {
    LoadCalculatorConfig();
    auto fix = nmr::test::TestEnvironment::FleetAmberTrajectory(kFixtureProtein);
    if (!FixtureAvailable(fix)) GTEST_SKIP() << "fixture not on disk";

    nmr::RunConfiguration config;
    config.SetName("TripeptideNeighborResidualVecPrevTimeSeriesFinalizeIdempotency");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac   = true;
    opts.skip_coulomb = true;
    opts.skip_apbs    = true;
...
TEST(TripeptideNeighborResidualVecPrevTimeSeries, H5RoundTrip) {
    ...
    const std::string h5_path = (fs::temp_directory_path() /
        ("tripeptide_neighbor_residual_vec_prev_ts_h5_roundtrip_" +
         std::to_string(::getpid()) + ".h5")).string();
    {
        HighFive::File file(h5_path, HighFive::File::Truncate);
        tr.WriteH5Group(tp, file);
...
// ============================================================================
// INTEGRATION + NaN-TOLERANT + LOG-OVERAGES.
//
// The producing calculator NaN-fills tripeptide_neighbor_residual_vec_prev
// at the top of each frame's Compute; finite values are stamped only
// where the i-1 tripeptide aligned + contributed. So this integration
// test:
//   - tallies finite vs NaN cells (NaN is signal, not corruption)
//   - asserts no Inf
//   - logs magnitude overages (>5 Å) on finite cells only
//   - population floor: some atoms must have finite, nonzero residuals
//     (N-terminal residue has no i-1 neighbour; ~all others should)
// ============================================================================

TEST(TripeptideNeighborResidualVecPrevTimeSeries, IntegrationLogOverages1P9J) {
    LoadCalculatorConfig();
    nmr::test::TestEnvironment::Load();
    if (nmr::RuntimeEnvironment::TensorCs15Dsn().empty()) {
    ...
    nmr::RunConfiguration config;
    config.SetName("TripeptideNeighborResidualVecPrevTimeSeriesIntegration");
    auto& opts = config.MutablePerFrameRunOptions();
    opts.skip_mopac   = true;
    opts.skip_coulomb = true;
    opts.skip_apbs    = true;
    ...
    // Walk every cell. Each component is either finite or NaN (NaN
    // signals "no i-1 contribution this frame"). Inf is a bug.
    ...
    for (std::size_t i = 0; i < buf->AtomCount(); ++i) {
        for (std::size_t t = 0; t < buf->StridePerAtom(); ++t) {
            ...
            const bool all_nan = std::isnan(v.x())
                              && std::isnan(v.y())
                              && std::isnan(v.z());
            // Reject Inf or mixed finite/NaN inside a single Vec3 — both
            // would indicate a writer bug.
            EXPECT_TRUE(all_finite || all_nan)
                << "atom " << i << " frame " << t << " has mixed "
                << "finite/NaN/Inf components: (" << v.x() << ", "
                << v.y() << ", " << v.z() << ")";
            if (all_finite) {
                ++finite_cells;
    ...
    if (overage_count > 0) {
        std::cout << "[warn] TripeptideNeighborResidualVecPrev: "
                  << overage_count << " finite cells exceed "
                  << kResidualMagWarnAngstrom << " Å (max norm "
                  << max_seen_magnitude << " Å) — chi-grid / perception edges, "
                  << "not asserted\n";
    }

    // Population floor on finite, nonzero cells at frame 0. N-terminal
    // residue has no i-1 neighbour, so a fraction will legitimately be
    // NaN; we still expect a majority finite.
    EXPECT_GT(finite_nonzero_at_frame0, buf->AtomCount() / 3)
        << "prev residual_vec finite-nonzero at frame 0: "
        << finite_nonzero_at_frame0 << " / " << buf->AtomCount()
        << " — perception or i-1 coverage drop?";

    std::cout << "TripeptideNeighborResidualVecPrev diagnostic: "
