...
std::unique_ptr<RingSusceptibilityShieldingTimeSeriesTrajectoryResult>
RingSusceptibilityShieldingTimeSeriesTrajectoryResult::Create(const TrajectoryProtein& tp) {
    auto r = std::make_unique<RingSusceptibilityShieldingTimeSeriesTrajectoryResult>();
...
void RingSusceptibilityShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).ringchi_shielding_contribution);
    }
...
void RingSusceptibilityShieldingTimeSeriesTrajectoryResult::Finalize(
    ...
    tp.AdoptDenseBuffer<SphericalTensor>(
        std::move(buffer),
        std::type_index(typeid(RingSusceptibilityShieldingTimeSeriesTrajectoryResult)));

    finalized_ = true;

    OperationLog::Info(LogCalcOther,
        "RingSusceptibilityShieldingTimeSeriesTrajectoryResult::Finalize",
...
void RingSusceptibilityShieldingTimeSeriesTrajectoryResult::WriteH5Group(
    ...
    auto grp = file.createGroup("/trajectory/ringchi_shielding_time_series");
