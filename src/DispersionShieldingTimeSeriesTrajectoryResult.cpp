...
std::unique_ptr<DispersionShieldingTimeSeriesTrajectoryResult>
DispersionShieldingTimeSeriesTrajectoryResult::Create(const TrajectoryProtein& tp) {
    auto r = std::make_unique<DispersionShieldingTimeSeriesTrajectoryResult>();
...
void DispersionShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).disp_shielding_contribution);
    }
...
void DispersionShieldingTimeSeriesTrajectoryResult::Finalize(
    ...
    tp.AdoptDenseBuffer<SphericalTensor>(
        std::move(buffer),
        std::type_index(typeid(DispersionShieldingTimeSeriesTrajectoryResult)));

    finalized_ = true;

    OperationLog::Info(LogCalcOther,
        "DispersionShieldingTimeSeriesTrajectoryResult::Finalize",
...
void DispersionShieldingTimeSeriesTrajectoryResult::WriteH5Group(
        const TrajectoryProtein& tp,
        HighFive::File& file) const {
    const auto* buffer =
        tp.GetDenseBuffer<SphericalTensor>(std::type_index(typeid(DispersionShieldingTimeSeriesTrajectoryResult)));
    if (!buffer) {
        OperationLog::Warn(
            "DispersionShieldingTimeSeriesTrajectoryResult::WriteH5Group",
    ...
    auto grp = file.createGroup("/trajectory/disp_shielding_time_series");
    ...
    grp.createAttribute("units",         std::string("Angstrom^-6"));
