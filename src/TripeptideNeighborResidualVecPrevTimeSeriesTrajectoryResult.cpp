...
void TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult::WriteH5Group(
    ...
    const auto* buffer =
        tp.GetDenseBuffer<Vec3>(std::type_index(typeid(TripeptideNeighborResidualVecPrevTimeSeriesTrajectoryResult)));
    if (!buffer) {
        OperationLog::Warn(
    ...
    if (source_present_count == 0) {
        OperationLog::Warn(
            ...
            " frames; skipping /trajectory/tripeptide_neighbor_residual_vec_prev_time_series/ "
    ...
    // through unchanged. Also NaN-fill rows where source wasn't
    // attached (absent-not-faked).
