...
// ── Compute ──────────────────────────────────────────────────────
//
// Phase 2b expansion (2026-05-17): tensor channels preserved through
...
void BsWelfordTrajectoryResult::Compute(
    ...
    for (size_t i = 0; i < N; ++i) {
        const SphericalTensor& st = conf.AtomAt(i).bs_shielding_contribution;
...
// ── WriteH5Group ─────────────────────────────────────────────────
//
// /trajectory/bs_welford/ — expanded schema (Phase 2b/C). Datasets:
//
...
//     t0_dxdt_{mean,m2,std,min,max,min_frame,max_frame}
//         — cadence-normalized rate (Δx / Δt). Stride-independent.
...
void BsWelfordTrajectoryResult::WriteH5Group(
    ...
    // units, *_delta_squared_* has squared units, *_dxdt_* has /ps
    // appended, *_min_frame and *_max_frame carry "frame_index").
    ...
    // min / max) carry base_units; *_m2 carries base_units^2 (squared
    // by construction of Welford M2). *_min_frame / *_max_frame carry
    // "frame_index" (dimensionless ordinal).
    ...
    // Per-dataset units identical to emit_1d but applied across 2D
    // (N, K) datasets. write_raw returns void in HighFive, so we
    // store the dataset handle then attach the attribute.
    ...
    emit_1d("t0",          kBaseUnits, kSquaredUnits,
            [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0; });
    emit_1d("t2magnitude", kBaseUnits, kSquaredUnits,
            [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t2magnitude; });

    // ── Per-component channels (T1, T2) ─────────────────────────
    emit_2d("t1", 3, kBaseUnits, kSquaredUnits,
            [&](size_t i, size_t k) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t1[k]; });
    emit_2d("t2", 5, kBaseUnits, kSquaredUnits,
            [&](size_t i, size_t k) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t2[k]; });

    // ── Delta variants (T0) ──────────────────────────────────────
    // Note: t0_delta_squared *values* are in base^2 (the Welford was
    // accumulated over Δ²); its _m2 dataset is therefore in base^4.
    ...
    emit_1d("t0_dxdt",          kRateUnits, kRateUnitsSq,
            [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0_dxdt; });
    ...
    // dxdt_n_per_atom: per-atom count of VALID-dt dxdt samples.
    // Equals delta_n on well-formed trajectories; diverges when frame
    // duplication / identical-timestamp rows are present (codex 2026-05-18).
    // Lets downstream verify dxdt sample population separately and
    // detect trajectories with high frame-duplication.
