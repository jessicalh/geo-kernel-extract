...
// ── WriteH5Group ─────────────────────────────────────────────────
//
// /trajectory/bs_welford/ — expanded schema (Phase 2b). Datasets:
//
...
void BsWelfordTrajectoryResult::WriteH5Group(
    ...
    emit_1d("t0",          [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0; });
    emit_1d("t2magnitude", [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t2magnitude; });

    // ── Per-component channels (T1, T2) ─────────────────────────
    emit_2d("t1", 3, [&](size_t i, size_t k) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t1[k]; });
    emit_2d("t2", 5, [&](size_t i, size_t k) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t2[k]; });

    // ── Delta variants (T0) ──────────────────────────────────────
    emit_1d("t0_delta",         [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0_delta; });
    emit_1d("t0_abs_delta",     [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0_abs_delta; });
    emit_1d("t0_delta_squared", [&](size_t i) -> const WelfordMoments& { return tp.AtomAt(i).bs_welford.t0_delta_squared; });
