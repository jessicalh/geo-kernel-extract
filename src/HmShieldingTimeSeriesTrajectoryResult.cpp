...
std::unique_ptr<HmShieldingTimeSeriesTrajectoryResult>
HmShieldingTimeSeriesTrajectoryResult::Create(const TrajectoryProtein& tp) {
    auto r = std::make_unique<HmShieldingTimeSeriesTrajectoryResult>();
...
void HmShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).hm_shielding_contribution);
    }
...
// we don't AdoptDenseBuffer.
void HmShieldingTimeSeriesTrajectoryResult::Finalize(TrajectoryProtein& tp,
                                                    Trajectory& traj) {
    (void)traj;
    ...
    tp.AdoptDenseBuffer<SphericalTensor>(
        std::move(buffer),
        std::type_index(typeid(HmShieldingTimeSeriesTrajectoryResult)));

    finalized_ = true;

    OperationLog::Info(LogCalcOther,
        "HmShieldingTimeSeriesTrajectoryResult::Finalize",
...
void HmShieldingTimeSeriesTrajectoryResult::WriteH5Group(
    ...
    const auto* buffer = tp.GetDenseBuffer<SphericalTensor>(std::type_index(typeid(HmShieldingTimeSeriesTrajectoryResult)));
    if (!buffer) {
        OperationLog::Warn(
            "HmShieldingTimeSeriesTrajectoryResult::WriteH5Group",
    ...
    auto grp = file.createGroup("/trajectory/hm_shielding_time_series");
