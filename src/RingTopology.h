...
public:
    ...
    // HIS variant_index 0/1 selects HID/HIE; 2 and unresolved/default
    // values select the ambiguous HisImidazole type.
    //
    // FATAL+abort on:
    ...
    // atom, one bonded to the Ortho2-labelled atom). TRP-9 also uses
    // the bond graph for its two BridgeFusion atoms. PHE / TYR /
    // HIS / Pro walks are label-only (with Locant disambiguating
    // HIS's two Heteroatom_* atoms).
    ...
    // ── Saturated rings (Pro pyrrolidine) ──
...
private:
    ...
    // (Ring.cpp:32-37). The full walk determines Ring::atom_indices
    // order and the derived RingGeometry::vertices order. Bless
    // bit-identity on the eight aromatic ring types plus Pro's
    // saturated ring is the gate.
    //
    ...
    //   - TRP-9 two BridgeFusion atoms: by bond-graph adjacency (the
    //     "first" is bonded to NE1; the "second" is bonded to CG).
    //
    // Each per-kind walker has a doc block with the AminoAcidType.cpp
    // atom-name order it reproduces and the disambiguation rule used.
