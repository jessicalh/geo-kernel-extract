...
std::unique_ptr<PiQuadrupoleShieldingTimeSeriesTrajectoryResult>
PiQuadrupoleShieldingTimeSeriesTrajectoryResult::Create(const TrajectoryProtein& tp) {
    auto r = std::make_unique<PiQuadrupoleShieldingTimeSeriesTrajectoryResult>();
...
void PiQuadrupoleShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).piquad_shielding_contribution);
    }
...
void PiQuadrupoleShieldingTimeSeriesTrajectoryResult::Finalize(
    ...
    tp.AdoptDenseBuffer<SphericalTensor>(
        std::move(buffer),
        std::type_index(typeid(PiQuadrupoleShieldingTimeSeriesTrajectoryResult)));

    finalized_ = true;

    OperationLog::Info(LogCalcOther,
        "PiQuadrupoleShieldingTimeSeriesTrajectoryResult::Finalize",
...
void PiQuadrupoleShieldingTimeSeriesTrajectoryResult::WriteH5Group(
    ...
    auto grp = file.createGroup("/trajectory/piquad_shielding_time_series");
    ...
    grp.createAttribute("units",         std::string("Angstrom^-5"));
