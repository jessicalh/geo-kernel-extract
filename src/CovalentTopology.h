...
// plus one set of positions. It returns a self-contained object.
//
...
public:
    ...
    //
    // If a pair's bond is missing from the geometric topology (the
    // active bond-perception path did not detect it), the bond is
    // force-added with BondOrder::Single + BondCategory::Disulfide
    // and the per-atom bond_indices_ updated.
    //
