...
void PrintUsage(const char* program_name) {
    (void)std::fprintf(stderr,
                       ...
                       "      run every calculator, emit per-atom NPYs.\n"
                       "\n"
                       "  --of3 --root NAME [--no-mopac]\n"
                       "      Load an OpenFold pose prepared with tleap/ff14SB. NAME expands to:\n"
                       "        {NAME}.prmtop   (topology + authoritative charges, required)\n"
                       "        {NAME}.inpcrd   (AMBER rst7 coordinates, required)\n"
                       "      Runs the standard single-pose stack; no ORCA/DFT or mutant\n"
                       "      comparison. A co-located {NAME}_nmr.out is ignored by construction.\n"
                       "\n"
                       ...
                       "\n"
                       "  --trajectory DIR [--stride N] [--window-start N --window-len M] [--mopac]\n"
                       ...
                       "        only cadence knob.\n"
                       "      --window-start N --window-len M: process only TRR frames [N, N+M)\n"
                       "        (window-LOCAL extraction); --stride still applies within the window;\n"
                       "        omit for the full trajectory. Both or neither.\n"
