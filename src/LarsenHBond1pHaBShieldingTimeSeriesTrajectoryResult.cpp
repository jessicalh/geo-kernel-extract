...
std::unique_ptr<LarsenHBond1pHaBShieldingTimeSeriesTrajectoryResult>
...
void LarsenHBond1pHaBShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).larsen_hbond_1pHaB_spherical);
    }
...
void LarsenHBond1pHaBShieldingTimeSeriesTrajectoryResult::Finalize(
    ...
    OperationLog::Info(LogCalcOther,
        "LarsenHBond1pHaBShieldingTimeSeriesTrajectoryResult::Finalize",
        "transferred (" + std::to_string(N) + " atoms x " +
        std::to_string(n_frames_) +
        " frames) 1pHaB shielding time-series to tp dense buffer");
}
...
void LarsenHBond1pHaBShieldingTimeSeriesTrajectoryResult::WriteH5Group(
    ...
    if (source_present_count == 0) {
        OperationLog::Warn(
            ...
            " frames; skipping /trajectory/larsen_hbond_1pHaB_shielding_time_series/ "
