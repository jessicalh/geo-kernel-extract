...
// a TripeptideDftRecord with the DFT row's atom positions, full
// asymmetric Mat3 shielding tensors (T0+T1+T2 preserved), parsed
// scalar/T2 tensor fields, a perceived LarsenTripeptide when
// perception succeeds, and the frame_type method discriminator.
//
...
// when the parent residue has chi3/chi4 defined.
//
...
struct TripeptideDftAtom {
    ...
    // Shielding tensor and scalar/T2 fields parsed from the DB row.
    Mat3   shielding_tensor = Mat3::Zero();   // full asymmetric 3x3, ppm
    double isotropic        = 0.0;            // T0 = trace/3, ppm
...
struct TripeptideDftRecord {
    ...
    // row fetch. Provides typed-identity-driven access to backbone
    // slots and sidechain atoms.
...
public:
    ...
    // angles. residue_letter is the 1-letter amino-acid code (e.g.
    // 'F' for PHE, 'S' for SER). Angles in degrees.
    //
    ...
    //
    // Returns an empty record (calc_id == 0) on miss and logs the miss
    // with the rounded grid coordinates. Fallback on a chi-specific
    // miss is delegated to the call site (the calculator re-tries
    // with fewer chi axes), per the Bundle B fail-loud discipline —
    // no silent degradation in this layer.
    //
    // n_chi_axes (default -1) overrides the default chi-axis count
    // for the residue. When ≥ 0, that many chi columns are requested,
    // capped to the residue's natural chi depth (and the chi values
    // beyond the included count are ignored). The calculator-side
    // fallback uses this to walk from the residue's natural chi depth
    // down to phi/psi-only when chi-specific lookups miss.
    //
