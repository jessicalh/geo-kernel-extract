#pragma once
//
// NamingRegistry: load-boundary residue-name translation and atom-name
// canonicalisation.
//
...
// sources happens explicitly in rule predicates and Resolve(), not by
// editing rules to play nice. See
...
//   ORCA:         prmtop atom names echoed by ORCA-output loaders
...
// the output side.
//
// This registry translates inbound names at load boundaries. Unknown
// residue names return an empty canonical name, and atom-name
// canonicalisation failures abort. No guessing.
//
// After translation, typed objects carry their properties. The registry
// keeps external string naming at the boundary before typed composition.
//
...
enum class ToolContext {
    ...
    Orca     = 3,    // ORCA DFT loaders echo prmtop/input atom names
...
enum class NamingSource : uint8_t {
    ...
    /// RTP deviation in fleet_amber TPRs". The CHARMM naming/output
    /// path is retired (2026-05-02 quarantined-legacy); the only live consumer
    ...
    /// PdbFileReader.cpp. Inputs from this source rarely need
    ...
    /// surface in OrcaRunLoader.cpp). prmtop-loaded names are
    ...
    // The CHARMM naming/output path is retired (2026-05-02 memory
    // entry `project_charmm_retired_amber_only_2026-05-02`); a future
    // full CHARMM naming path can be added with a non-ambiguous source
    // tag at that time. The numeric value 5 is
