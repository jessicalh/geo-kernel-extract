...
// Dependencies: none.
//
// Charges come from a typed ChargeSource: ff14SB param file, AMBER
// prmtop, runtime tleap-prepared prmtop, or caller-preloaded values.
// The ChargeSource determines the force field; this result projects the
// prepared Protein charge table onto ConformationAtom.
//
// For protonation variants (HID/HIE/HIP, ASH, GLH, CYX/CYM, LYN,
// ARN, TYM): the ff14SB param file path uses the residue's
// protonation_variant_index to select the correct charge set. Other
// sources (tpr, prmtop) carry the protonation in their topology already.
//
