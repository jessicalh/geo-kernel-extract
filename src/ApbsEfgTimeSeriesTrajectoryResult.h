...
// attached in PerFrameExtractionSet (RunConfiguration.cpp:157), so
// in production the HasResult<ApbsFieldResult>() gate is defensive
// and source_attached_per_frame is all-1. The gate + per-frame mask
...
//
// Emission (T2-only — APBS EFG is the symmetric-traceless Hessian
// of φ; T0 and T1 are structurally zero from Hessian symmetry plus
// Poisson tracelessness, per the 2026-05-18 EFG schema rev,
// task #166):
//
...
//                                  faked'.
//
// Parity "2e": EFG is the Hessian of the electric potential φ. φ is
// parity-even (scalar), gradient (E) is 1o (polar vector), Hessian
// (EFG) is 2e (rank-2 even-parity tensor — same parity convention
// as the stress tensor). T0 and T1 are structurally zero from
// Hessian symmetry and the Poisson tracelessness projection
// (applied at source in ApbsFieldResult.cpp); only 5 T2 components
// are emitted.
//
