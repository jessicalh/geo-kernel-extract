...
// and is covered by tests that fetch one row from each DB
// (tripeptide, frame_type) combination plus the AAA Gaussian log. The
// Python script remains a useful diagnostic but is
...
// Order the pieces along the directed amide chain ACE → NCapAla → Central
// → CCapAla → NME. Returns the chain it can walk; the caller requires
// exactly 5 pieces.
...
std::string ParentHeavyForH(const std::string& h_name,
    ...
    // Polar-H cases are interleaved where their prefixes overlap
    // carbon-bound H names.
...
CanonicalPiece CanonicalHisVariant(const std::string& variant_name) {
    ...
    // (0=HID, 1=HIE, 2=HIP), matching AminoAcidType variant ordering.
...
CanonicalPiece CanonicalResidue(AminoAcid aa) {
    ...
    // Non-HIS residues are stamped with the base row here. HIS variants
    // are stamped from CanonicalHisVariant.
...
// ============================================================================
// WL signature matching (K=3 Weisfeiler-Lehman)
// ============================================================================
//
...
// happens to look identical. The canonical case is a PHE aromatic ring:
// CD1, CD2, CE1, CE2, and CZ all carry signature
// (C, 3, [(C,3), (C,3), (H,1)]) — 5 chemically-different atoms in a
// single class. TYR has the same symmetric CD/CE pairs, while CZ is
// distinguished by its OH neighbour. Zip-assigning canonical names within
// a collapsed class scrambles tensors across locants (perceived CE2's
// tensor lands on protein CD1).
//
...
// signatures. After K=3, the ipso, ortho-pair, meta-pair, and para
// classes are separated because their 3-hop neighbourhoods differ
// (CG's neighbour CB has degree 4; CZ's neighbours are aromatic Cs with
// degree-3 only; etc.). The remaining residual classes are the
// graph-symmetric pairs:
...
//
// Cost is bounded by small fixed piece sizes and is negligible for
// per-record perception.
...
// are comparable — which is what MatchPiece requires. At tens of atoms
// per piece the collision risk from the truncation is negligible.
...
//     (a WL-equivalent pair such as PHE CD1/CD2 that K rounds of
...
void StampChainIdentitiesViaTable(CanonicalPiece& p,
    ...
    for (int i = 0; i < static_cast<int>(p.atoms.size()); ++i) {
        ...
        // ComposeAtomSemantic's methyl-H collapse.
        ...
        // construction; copying from the row keeps this path aligned
        // with table-side canonical identity.
        // A lookup miss is unreachable from the standard-20 substrate
        // when the suite is green: LarsenResiduePerceptionTest
        // .AllCombinationsPerceiveCleanly fetches one row for each of
        // the 20 (residue, frame_type) combinations, and the companion
        // source-log test covers the AAA Gaussian log. Either would
...
bool EmitPiece(LarsenResidue::Kind kind,
    ...
    for (int local_idx = 0; local_idx < static_cast<int>(ordered.size());
        ...
        // identity is the AtomMechanicalIdentity stored on the
        // CanonicalAtom.
    ...
    for (int i : piece_atoms) {
        for (int j : full_adj[i]) {
            ...
            if (i < j) {
                ...
                b.a = dft_to_local[i];
                b.b = dft_to_local[j];
                b.order = 1;  // heuristic order only
