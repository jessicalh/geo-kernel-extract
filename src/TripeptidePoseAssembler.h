...
//   in the central-residue assembly path. The cap path reads the
//   typed N/H/CA/HA/CB/C/O slots from LarsenResidue directly.
...
//            residue i. Cap is always ALA; N/H/CA/HA/CB/C/O slots
//            are emitted when present on both sides.
...
//            always ALA; N/H/CA/HA/CB/C/O slots are emitted when
...
struct AlignedDftAtom {
    int         dft_atom_idx     = -1;          // diagnostic DFT atom identifier
    ...
    // Substrate cross-check outcome. On the cap path, true iff the
    // protein atom's typed substrate role (BackboneRole / Locant)
    // matches the role implied by the canonical DFT cap position; a
    // false cap atom is emitted only when substrate_check_strict=false.
    // On the central path, a typed-identity candidate match sets this
...
struct AssembledTripeptide {
    bool ok = false;                          // false when no atoms are emitted
    int  calc_id = 0;
    ...
    std::vector<AlignedDftAtom> aligned_atoms;  // atoms emitted by the selected path

    // Aggregate diagnostics for this assembly attempt (both emitted and
    // rejected atoms contribute to counters).
    int    n_substrate_disagreements = 0;     // typed-substrate match failures
...
//     aligned_atoms. The cap path's seven slots come from typed
//     LarsenResidue slots and protein-side substrate role checks, so a
//     large residual on a BB/Cβ atom indicates a Kabsch failure or a
//     substrate disagreement worth excising.
...
//
// Returns AssembledTripeptide with ok=false when no atoms can be emitted:
//   - DFT record is absent
//   - Protein residue's backbone N/CA/C cache incomplete
//   - LegacyAmber atom semantics are absent for typed matching
//   - Perceived LarsenTripeptide is absent or missing required slots
//   - All candidate atoms are rejected or unmatched
