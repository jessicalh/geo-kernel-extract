...
std::unique_ptr<LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult>
...
void LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::Compute(
    ...
    for (std::size_t i = 0; i < N; ++i) {
        per_atom_shielding_[i].push_back(
            conf.AtomAt(i).larsen_hbond_1pHB_spherical);
    }
...
void LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::Finalize(
    ...
    OperationLog::Info(LogCalcOther,
        "LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::Finalize",
        "transferred (" + std::to_string(N) + " atoms x " +
        std::to_string(n_frames_) +
        " frames) 1pHB shielding time-series to tp dense buffer");
}

void LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::WriteH5Group(
        const TrajectoryProtein& tp,
        HighFive::File& file) const {
    const auto* buffer =
        tp.GetDenseBuffer<SphericalTensor>(std::type_index(typeid(LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult)));
    if (!buffer) {
        OperationLog::Warn(
            "LarsenHBond1pHBShieldingTimeSeriesTrajectoryResult::WriteH5Group",
    ...
    if (source_present_count == 0) {
        OperationLog::Warn(
            ...
            " frames; skipping /trajectory/larsen_hbond_1pHB_shielding_time_series/ "
