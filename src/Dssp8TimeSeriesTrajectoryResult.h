...
public:
    ...
    // it passes to Compute before setting this flag. (Larsen-family
    // TRs read a per-atom field instead of .Result<>() and are safe;
    // THIS TR reads the Result object, so the constraint is real.)
