...
void SasaWelfordTrajectoryResult::Compute(
    ...
    // dxdt uses its OWN counter (sasa_welford.dxdt_n) to skip zero-dt
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        if (prev_valid_[i]) {
            ...
            if (std::abs(dt) > MIN_DT_PS) {
                ...
                WelfordUpdate(w.sasa_dxdt, delta_x / dt, dxdt_n_new, frame_idx);
                w.dxdt_n = dxdt_n_new;
            }
        }
        prev_value_[i] = s;
...
void SasaWelfordTrajectoryResult::WriteH5Group(
    ...
    // dxdt_n_per_atom: per-atom count of VALID-dt sasa_dxdt samples.
