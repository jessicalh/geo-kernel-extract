...
Mat3 SphericalTensor::Reconstruct() const {
    ...
    double const Sxy = T2[0] * INV_SQRT2;
    double const Syz = T2[1] * INV_SQRT2;
    double const Szz = T2[2] * INV_SQRT3_2;
    double const Sxz = T2[3] * INV_SQRT2;
    double const Sxx_m_Syy = T2[4] * SQRT2;

    // Recover Sxx and Syy from Szz and (Sxx - Syy), using tracelessness:
    //   Sxx + Syy + Szz = 0
    //   Sxx - Syy = Sxx_m_Syy
    double const Sxx = (-Szz + Sxx_m_Syy) / 2.0;
    double const Syy = (-Szz - Sxx_m_Syy) / 2.0;
