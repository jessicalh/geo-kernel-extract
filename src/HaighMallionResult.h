#pragma once
//
// HaighMallionResult: surface integral ring current shielding kernels.
//
// For each aromatic ring within the configured cutoff (default 15 A), computes:
//
...
//      (Stroud T2:5-1 / Dunavant degree-5). Adaptive subdivision uses
//      configured thresholds (defaults 2.0 A for level 1, 1.0 A for level 2).
...
//      G is RANK-1 (like Biot-Savart), and its decomposition can contain
//      T0, T1, and T2 components.
//      Stored as hm_G_tensor on RingNeighbourhood.
//
//      Sign convention: the minus comes from the shielding tensor
//      definition sigma_ab = -dB_a^sec/dB_{0,b}. Same convention as
//      BiotSavartResult (see BiotSavartResult.h): sigma = I * G gives
//      the correct sign with literature ring current intensities
//      (I < 0 for diamagnetic).
//
...
// Both produce rank-1 shielding kernels (G = -V (x) n, i.e.
// G_ab = -V_a n_b), but compute the "effective B-field" V differently.
// Whether they agree on T2 is the empirical question.
//
