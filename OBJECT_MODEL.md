...
| ✓ | `HydrationShellTimeSeriesTrajectoryResult` | HydrationShell | FO | per-atom (N, T) — 4 scalar channels: half_shell_asymmetry (COM frame), mean_water_dipole_cos, nearest_ion_distance (+inf sentinel on no-ion-in-cutoff), nearest_ion_charge. Source-attached gate; reference_frame="COM"; nearest_ion_charge_zero ambiguous with no-ion case (per joint-attr disambiguation). Commit f15b33a. |
| ✓ | `HydrationShellWelfordTrajectoryResult` | HydrationShell | AV | per-atom Welford rollup; 4 scalar channels with full delta variants + `ion_present_fraction` indicator-Welford (no deltas). `nearest_ion_distance` uses CONDITIONAL Welford (R6 codex 2026-05-18): only finite samples contribute; per-atom counter `n_ion_present_per_atom` is Finalize divisor; delta variants gated on BOTH prev+curr finite (`n_ion_delta_per_atom`); dxdt likewise (`n_ion_dxdt_per_atom`). Atoms with zero in-cutoff frames finalize to NaN (n=0). For "Pr(ion in cutoff)" use `ion_present_fraction.mean ∈ [0, 1]`. Commit f15b33a + R6 follow-up. |
| ✓ | `HydrationGeometryTimeSeriesTrajectoryResult` | HydrationGeometry | FO | per-atom (N, T, 3)/(N, T) — Vec3 dipole_vector (e·Å units, NOT Debye) + Vec3 surface_normal + uint32 first_shell_count + 3 polarisation scalars: alignment (cos angle), coherence (e·Å — source formula `\|Σd\|/n_shell` is NOT a [0,1] order parameter, R6 codex 2026-05-18), asymmetry (fraction). polarisation_signal_channels attr names the trio; reference_frame="SASA_normal". Source-attached gate on `HasResult<HydrationGeometryResult>()`. Commit 6931528 + R5/R6 units fixes. |
| ✓ | `HydrationGeometryWelfordTrajectoryResult` | HydrationGeometry | AV | per-atom Welford rollup; per-component Vec3 + 4 scalar Welford channels (asymmetry / alignment / coherence / shell_count) each with full delta variants. dipole_coherence channel units e·Å (NOT order_parameter, R6 codex 2026-05-18). Source-attached gate skips Welford updates + invalidates prev_* on absent frames; mean_dt_ps + frame_index_range from attached-subset. Dipole units e·Å. Commit 6931528 + R5/R6 units fixes. |
| ✓ | `AIMNet2EmbeddingTimeSeriesTrajectoryResult` | AIMNet2 | FO | per-atom (N, T, 256) float32 — `aimnet2_aim` learned feature vector. Per-atom hyperslab WriteH5Group (no flat-buffer doubling). Attrs `irrep_layout="feature_vector"`/`parity="0e"`; `optional_large=true`. Source policy `always_attached` but compute-time `conf.HasResult<AIMNet2Result>()` gate emits NaN-placeholder + `source_attached_per_frame=0` on absent frames (canonical "absent, not faked" gate; mask honestly reflects per-frame attach state, not hardcoded all-1). SDK loader `load_optional_large=False` by default skips the heavy dataset (~3-4 GB/protein); set True to load. Commit 510681f → fac1e46. |
| ✓ | `AIMNet2ChargeResponseGradientTimeSeriesTrajectoryResult` | AIMNet2ChargeResponseGradient | FO | per-atom (N, T, 3) double `charge_response_gradient_vector` (e²/Å — gradient of L = Σ_j q_j² via torch autograd backward through AIMNet2 charge head) + (N, T) double `charge_response_gradient_scalar` (L2 norm, e²/Å). Both emitted per methods-accumulate. Attrs follow existing Vec3 TR convention: `irrep_layout_vector="x,y,z"`/`normalization_vector="cartesian"`/`parity_vector="1o"`; `irrep_layout_scalar="T0"`/`parity_scalar="0e"`. AIMNet2ChargeResponseGradientResult RequireConformationResult'd alongside AIMNet2Result (always-attached); compute-time `conf.HasResult<AIMNet2ChargeResponseGradientResult>()` gate emits NaN-placeholder + `source_attached_per_frame=0` on absent frames. The emitted quantity is the gradient of a sum-of-squared-charges scalar, NOT a Buckingham polarisability α = ∂μ/∂E — rename science S2 2026-05-20. |
| ✓ | `AIMNet2ChargeResponseGradientWelfordTrajectoryResult` | AIMNet2ChargeResponseGradient | AV | per-atom Welford rollup of the AIMNet2 charge-response gradient (TR #3 of the fleet trio). Emits full canonical Welford row per sibling TR convention: `vector_{mean,std,m2,min,max,min_frame,max_frame}` (N, 3) + `scalar_{mean,std,m2,min,max,min_frame,max_frame}` (N,) + `n_per_atom` (N,) uint64. Per-dataset units attrs: base channel `e^2/Å` for mean/std/min/max; `(e^2/Å)^2` for m2; `frame_index` for *_frame; `frame_count` for n_per_atom (codex H3 2026-05-20). Same gate as the TS pair (HasResult skip + mask=0); F1 non-finite-gradient guard at the source calculator also feeds this gate. Minimum-viable v0 — no delta variants. Group is skipped entirely on `source_attached_count==0`. SDK: `AIMNet2ChargeResponseGradientWelfordGroup`. |
| ✓ | `ApbsEfgTimeSeriesTrajectoryResult` | ApbsField | FO | per-atom (N, T, 5) double — five T2 components (T2_m-2..T2_m+2) of the APBS electric field gradient `ConformationAtom.apbs_efg_spherical`, units V/Å². T2-only per the 2026-05-18 EFG schema rev (task #166): APBS EFG = Hessian of φ from the linearised Poisson-Boltzmann solve is symmetric-traceless after the source-side projection, so T0+T1 are structurally zero. Attrs `irrep_layout="T2_m-2,T2_m-1,T2_m0,T2_m+1,T2_m+2"` / `normalization="isometric_real_sph"` / `parity="2e"` (Hessian of a parity-even scalar is rank-2 even). ApbsFieldResult is `RequireConformationResult`'d in PerFrameExtractionSet (RunConfiguration.cpp:157) — `source_attached_per_frame` is all-1 in production. Compute-time `HasResult<ApbsFieldResult>()` gate emits NaN-fill + mask=0 on absent frames for SDK uniformity per OBJECT_MODEL "Conditional-attach TR discipline". TR4 of the 13-TR plan; canonical T2 EFG TS — sibling of `ApbsEfieldTimeSeriesTrajectoryResult` (Vec3 TS, same source calc). SDK: `ApbsEfgTimeSeriesGroup`. |
| ✓ | `MopacChargeWelfordTrajectoryResult` | Mopac | AV | per-atom Welford rollup of MOPAC Mulliken charges (`ConformationAtom.mopac_charge`, units e — elementary charge). Single channel, full canonical row: `charge_{mean,std,m2,min,max,min_frame,max_frame}` (N,) + `n_per_atom` (N,) uint64. Per-dataset units: `e` for mean/std/min/max, `e^2` for m2, `frame_index` for *_frame, `frame_count` for n_per_atom (codex H3 2026-05-20 convention). MopacResult attaches **conditionally / sparsely** via `TimedAttach` in OperationRunner.cpp:142 (NOT Require) — Mopac cadence (~20 ps in production) is JobSpec-driven and TR-agnostic. Compute's `HasResult<MopacResult>()` gate skips the Welford update + records mask=0 on absent frames; per-atom `n_frames` counter advances only on attached frames so WelfordFinalize sees the correct divisor. When `source_attached_count==0` (MOPAC disabled for this run), `WriteH5Group` skips the entire `/trajectory/mopac_charge_welford/` group per canonical "absent, not faked" — SDK readers must tolerate group absence as "MOPAC disabled". Minimum-viable v0 — no delta variants (precedent: AIMNet2 ChargeResponseGradient Welford v0; charges are continuous, charge_std already captures fluctuation). TR5 of the 13-TR plan; canonical sparse-Welford-scalar — TR6 (MopacBondOrderWelford) clones this shape against the bond axis. SDK: `MopacChargeWelfordGroup`. |
| ✓ | `MopacBondOrderWelfordTrajectoryResult` | Mopac | AV | per-bond Welford rollup of MOPAC Wiberg bond orders (`MopacResult.TopologyBondOrders()`, dimensionless). Bond axis B = `protein.BondCount()` parallel to `bonds.npy`. Per-bond Welford state TR-internal (BondLengthStats pattern). **Sentinel-aware Welford** per `feedback_conditional_welford_for_sentinels` (landed 2026-05-21 per S2 adversarial review M6/M4): MopacResult sets `bo = 0.0` exactly for bonds it didn't report (the "no observation" sentinel for a continuous QM observable). Naive accumulation biases the running mean toward 0 for intermittently-reported bonds (typical for MOZYME-merged sidechain interior bonds). The order Welford updates ONLY when `bo != 0.0` (divisor = `per_bond_n_present`); a companion `order_present_fraction` indicator-Welford updates EVERY MOPAC-attached frame with a 1.0/0.0 indicator (divisor = `source_attached_count` = `n_total_per_bond`, group attr). Same sparse-cadence + "absent, not faked" gate as TR5. Group attr `bond_axis = "bonds.npy"`. TR6 of the 13-TR plan. SDK: `MopacBondOrderWelfordGroup` (includes the `order_present_fraction_*` row). |
| ✓ | `MopacCoulombShieldingTimeSeriesTrajectoryResult` | MopacCoulomb | FO | per-atom (N, T, 5) double — five T2 components of the MOPAC Coulomb **EFG kernel** (`ConformationAtom.mopac_coulomb_shielding_contribution`, bare EFG in V/Å²; despite the field name "shielding_contribution", NO γ multiplication is applied at extraction — γ × T2 → ppm-shielding is downstream calibration). T2-only emission per the MopacCoulombResult.cpp:252-254 source comment "Pure T2 (EFG is traceless). gamma converts this to shielding." — EFG_total is symmetric traceless from the Poisson tracelessness projection at source, so T0+T1 are structurally zero. Attrs `irrep_layout="T2_m-2,T2_m-1,T2_m0,T2_m+1,T2_m+2"` / `normalization="isometric_real_sph"` / `parity="2e"` / `units="V/Å^2"` (corrected 2026-05-21 per S2 adversarial science/math H1; previously mislabeled "ppm"). MopacCoulombResult attaches via `TimedAttach` (OperationRunner.cpp:183, NOT Require) — same sparse cadence + "absent, not faked" gate as TR5/TR6. TR7 of the 13-TR plan; combines TR4 T2-only emission with TR5 sparse-cadence gate. SDK: `MopacCoulombShieldingTimeSeriesGroup`. |
| ✓ | `MopacMcConnellShieldingTimeSeriesTrajectoryResult` | MopacMcConnell | FO | per-atom (N, T, **9**) double — all T0+T1+T2 components of the MOPAC-weighted McConnell bond-anisotropy **kernel** (`ConformationAtom.mopac_mc_shielding_contribution`, bare bond-order-weighted `bo·M/r³` in Å⁻³; NO Δχ × γ at extraction). **9-component emission** deliberately diverges from the 13-TR plan's "(N, T, 5)" — the source M_total accumulates the bond-anisotropy kernel `M_ab = 9·cos_θ·d̂_a·b̂_b - 3·b̂_a·b̂_b - (3·d̂_a·d̂_b - δ_ab)` which is **not symmetric-traceless**. Per user direction 2026-05-21 "if not traceless write both" + methods-accumulate. **T0 = bond-order-weighted McConnell f-scalar sum** (= sum of `mopac_mc_co_sum + mopac_mc_cn_sum + mopac_mc_sidechain_sum + mopac_mc_aromatic_sum`); **T1 = antisymmetric McConnell pseudovector** from the cross-coupling outer-product terms (real geometric quantity); **T2 = symmetric traceless McConnell tensor** (canonical bond-anisotropy contribution) — see source attr for the per-channel physics. Integration1P9J shows max\|T0\|≈0.96 Å⁻³, max\|T1\|≈1.14 Å⁻³, max\|T2\|≈0.98 Å⁻³, all three channels carry signal. Attrs `irrep_layout="T0,T1_m-1,T1_m0,T1_m+1,T2_m-2,T2_m-1,T2_m0,T2_m+1,T2_m+2"` / `parity="0e+1o+2e"` (matches FF14SB McConnellShieldingTimeSeries) / `units="Angstrom^-3"` (corrected 2026-05-21 per S2 adversarial science/math H1; previously mislabeled "ppm"). Same sparse-cadence gate as TR5/TR6/TR7. TR8 of the 13-TR plan. SDK: `MopacMcConnellShieldingTimeSeriesGroup`. |
| ✓ | `MopacVsFf14SbReconciliationTrajectoryResult` | MopacCoulomb + Coulomb | FO | per-atom-per-frame (N, T) double — `cos(MOPAC_T2, FF14SB_T2)` ∈ **[-1, 1] (SIGNED)** in the T2 5-vector subspace; cross-source comparison of Coulomb EFG kernels from MOPAC PM7+MOZYME Mulliken charges vs FF14SB-parameterised charges. **Signed cosine** per S2 adversarial science review M1 (landed 2026-05-21): the T2 5-vector is sign-deterministic, so opposite-polarisation is a real physical disagreement (e.g., MOPAC vs FF14SB charge-sign flip at SER OG, ARG NH2). Prior `|cos|` collapsed this signal. Integration1P9J empirical: mean(cos)=0.75, range=[-0.98, +1.00], 85/846 atoms negative cosine — exactly the chemistry-driven disagreement the calibration ridge needs to see. Uses `SphericalTensor::T2CosineWith()` method (refactored from anonymous-namespace free function per anti-patterns review H1). **Cross-source gate**: requires BOTH `MopacCoulombResult` AND `CoulombResult` attached per frame; either-absent → all-N NaN cells + mask=0. **Per-atom NaN** when either-side |T2| < `coulomb_efg_t2_magnitude_floor` (CalculatorConfig key, 1e-3 V/Å²; calibrated to EFG signal scale NOT the project-wide direction-vector 1e-10 which would let FP-noise-dominated atoms through — math adversarial review H1). WriteH5Group skips the entire `/trajectory/mopac_vs_ff14sb_reconciliation/` group when no frame had both attached. Registered ONLY in `FullFatFrameExtraction` (where both `skip_mopac=false` and `skip_coulomb=false` — moved from `PerFrameExtractionSet` 2026-05-21 per S2 adversarial review H3; was structurally unreachable in production). Dataset name `cos_t2` (renamed from `abs_cos_t2`). SDK: `MopacVsFf14SbReconciliationGroup` with `.per_atom_mean_cos()` + `.per_atom_finite_count()` convenience methods. TR9 of the 13-TR plan; new cross-source pattern, its own canonical. |
...
| ✓ | `DihedralBinTransitionTrajectoryResult` | (none — positions + AminoAcidType.chi_angles + Protein backbone bond graph) | AV | per-residue stats in `/trajectory/dihedral_bin_transition/`: `backbone_transition_count` (R,), `backbone_dominant_region` (R,), `n_frames_observed` (R,), `backbone_bin_occupancy` (R, 6), `chi_transition_count` (R, 4), `chi_dominant_rotamer` (R, 4), `chi_n_frames_observed` (R, 4), `chi_rotamer_occupancy` (R, 4, 3). Bin labels match DihedralTS verbatim (6-region Rama grid + αR→αL→PPII→β→other resolution order; 3-bin chi rotamer g+/t/g−). Consecutive-frame transition gate (BOTH prev and curr observed). Pure AV: no DenseBuffer, running counters update each Compute. Independent compute from positions + bond graph; does not cross-read DihedralTS. Wrap-correct for cyclic peptides via the same Predecessor/Successor walk. |
| ✓ | `Dssp8TimeSeriesTrajectoryResult` | DsspResult (conditional) | FO | per-residue (R, T) — `ss8_code` uint8 (8-state H/G/I/E/B/T/S/C with 255=unassigned) + `hbond_acceptor_partner` (R, T, 2) int32 / `hbond_acceptor_energy` (R, T, 2) float64 kcal/mol + `hbond_donor_partner` / `hbond_donor_energy` (same shape). Static `residue_index_per_atom` (N,) for atom broadcast. Conditional source: DsspResult attaches only when `skip_dssp` is false; source-attached gate emits all-1 mask when present, skips group entirely when never attached. Phi/psi/SASA from DsspResult NOT mirrored — phi/psi live in DihedralTS (IUPAC, negated relative to DSSP's libdssp convention). |
| ✓ | `Dssp8TransitionTrajectoryResult` | DsspResult (conditional) | AV | per-residue stats in `/trajectory/dssp8_transition/`: `ss8_transition_count` (R,), `ss8_dominant` (R,), `n_frames_observed` (R,), `ss8_occupancy` (R, 8), `ss8_transition_matrix` (R, 8, 8) — diagonal identically zero (self-transitions excluded). Consecutive-frame gate; source-absent frames break the chain. Pure AV: running counters update each Compute. |
| ✓ | `BondedEnergyTimeSeriesTrajectoryResult` | BondedEnergy | FO | per-atom (N, T) — 7-channel CHARMM36m decomposition: bond/angle/urey_bradley/proper_dih/improper_dih/cmap_dih/total. Protein-slice only (`system_scope` attr; sum ≠ whole-system .edr term). Source-attached gate on `HasBondedParams()`. Commits 322d465 + aa7bb31 + d3209a0 |
...
| ✓ | `RingNeighbourhoodTrajectoryStats` | (none — reads positions + GeometryResult.ring_geometries + SpatialIndexResult; PATTERN A3 scope, NOT the original Pattern A fat-union plan) | FO | per-(atom, aromatic-ring) geometric residual in `/trajectory/ring_neighbourhood_trajectory_stats/`: static `ring_membership_per_atom (N, R_per_atom_max) int32` (frame-0 snapshot of which aromatic rings sit within `ring_current_spatial_cutoff` 15 Å of each atom; -1 sentinel for unfilled slots; sorted ascending) + dynamic `geometry (N, T, R_per_atom_max, 4) float64` per-frame 4-channel (distance, rho, z, in_plane_angle) with channel_layout / units / cutoff / NaN-semantics attrs. ProPyrrolidine excluded (saturated rings emit via RingPuckerTimeSeries). Group SKIPPED when r_per_atom_max=0 (no aromatic-ring/atom pairs in protein). Source policy `always_attached` (positions present at tp.Seed); `source_attached_per_frame` all-1 + policy attr for SDK uniformity. Independent of the 5 ring calcs' fat-union `ca.ring_neighbours` (computes geometry FRESH per frame from `conf.ring_geometries[ri]` + atom positions) — see `project_ring_neighbourhood_debt_2026-05-21` for the deferred fat-union cleanup. TR10 of the 13-TR plan. SDK: `RingNeighbourhoodTrajectoryStatsGroup` (with `.distance() / .rho() / .z() / .in_plane_angle() / .live_slots_mask()` accessors). |
...
| ✓ | `RmsdTrackingTrajectoryResult` | positions only (no source ConformationResult); reads Residue.N/CA/C/O cached indices | AV (scalar (T,) double) | per-frame Kabsch-aligned backbone-RMSD (N/CA/C/O heavy atoms) vs trajectory frame 0, in `/trajectory/rmsd_tracking/`. First scalar AV (T,) TR in the codebase. Datasets: `rmsd` (T,) float64 Å + `atom_indices` (M,) int32 (alignment set provenance) + frame metadata. Frame 0 RMSD = 0 by construction (reference is self). Always-on source: source_attached_per_frame all 1. Cross-result-read WRITER side: TR11 exposes `RmsdAtFrame(frame_idx)` for downstream TRs (see `RmsdSpikeSelectionTrajectoryResult`). TR11 of the 13-TR plan. SDK: `RmsdTrackingGroup`. |
| ✓ | `RmsdSpikeSelectionTrajectoryResult` | `RmsdTrackingTrajectoryResult` (CROSS-RESULT READ on `RmsdAtFrame`) | AV (SelectionBag emitter) | Per-frame RMSD-spike detector. Dual-OR threshold: (rmsd > 2.0 Å absolute) OR (\|rmsd - rolling_100_frame_mean\| > 0.5 Å local-delta); cooldown 100 frames after each spike. Rolling-window criterion suppressed until ≥ 10 frames observed. Project-decision parameters (per 13-TR plan, not literature). Emits `SelectionRecord` entries to `traj.MutableSelections()` under own kind; no private WriteH5Group (SelectionBag writer emits `/trajectory/selections/<kind>/`). Records carry `rmsd_A`, `rolling_mean_A`, `local_delta_A`, `window_frames`, `abs_threshold_A`, `local_threshold_A`, `trigger` in metadata. TR12 of the 13-TR plan. |
...
| `DihedralTimeSeriesTrajectoryResult` | FO | none — reads positions + `Residue.chi[k]` (cached atom indices from `AminoAcidType.chi_angles`) + chain structure | per-residue datasets in `/trajectory/dihedral_time_series/`: phi / psi / omega / omega_deviation `(R, T)` float64 radians; chi `(R, T, 4)` radians; rama_region `(R, T)` uint8 (5-region literature grid); static per-residue masks (`chi_exists`, `omega_is_xpro`, `is_glycine`, `is_proline`, `is_pre_proline`, `residue_terminal_state`, `chain_id_per_residue`); atom-axis broadcast lookup `residue_index_per_atom (N,)`. IUPAC dihedral sign convention; NaN at termini + chain breaks; PHE/TYR + ASP/GLU symmetry caveats documented in attrs. Movie-target TR; per-frame resolution, no source-attached gate (positions always present). |
...
**Canonical exemplars (2026-05-21):** Future TRs implementing the
source-attached gate should follow these reference implementations
rather than re-deriving the discipline:

- **Conditional scalar Welford on TrajectoryAtom**:
  `MopacChargeWelfordTrajectoryResult` — sparse-cadence
  HasResult gate, per-atom `n_frames` advances only on attached
  frames; WriteH5Group skip on `source_attached_count==0`.
- **Conditional sentinel-aware per-bond Welford (TR-internal)**:
  `MopacBondOrderWelfordTrajectoryResult` — per-bond Welford
  state lives inside the TR (BondLengthStats pattern); sentinel
  detection (`bo != 0.0`) gates the order Welford while an
  indicator-Welford companion tracks presence_fraction. Mirrors
  the R6 `nearest_ion_distance` pattern from
  `feedback_conditional_welford_for_sentinels`.
- **Conditional SphericalTensor T2-only TS**:
  `MopacCoulombShieldingTimeSeriesTrajectoryResult` — emits
  T2-only (5-component) when source is structurally traceless;
  HasResult gate emits NaN-fill on absent frames.
- **Conditional SphericalTensor full-9-component TS**:
  `MopacMcConnellShieldingTimeSeriesTrajectoryResult` — emits all
  9 components when source has nonzero T0/T1 (asymmetric kernel).
- **Cross-source conditional comparison**:
  `MopacVsFf14SbReconciliationTrajectoryResult` — requires BOTH
  source ConformationResults; either-absent → all-N NaN cells;
  group-skip on `source_attached_count==0`. Uses
  `SphericalTensor::T2CosineWith()` per "objects answer questions
  about themselves" (PATTERNS Anti-Patterns).
