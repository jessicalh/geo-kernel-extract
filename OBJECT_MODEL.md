...
| ✓ | `HydrationShellTimeSeriesTrajectoryResult` | HydrationShell | FO | per-atom (N, T) — 4 scalar channels: half_shell_asymmetry (COM frame), mean_water_dipole_cos, nearest_ion_distance (+inf sentinel on no-ion-in-cutoff), nearest_ion_charge. Source-attached gate. |
| ✓ | `HydrationShellWelfordTrajectoryResult` | HydrationShell | AV | per-atom Welford rollup; 4 scalar channels with full delta variants. nearest_ion_distance degrades to NaN on atoms without ions (IEEE 754: inf-inf=NaN) — downstream `np.isfinite()` filters cleanly. |
