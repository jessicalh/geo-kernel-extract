...
| ✓ | `AIMNet2ChargeResponseGradientWelfordTrajectoryResult` | AIMNet2ChargeResponseGradient | AV | per-atom Welford rollup of the AIMNet2 charge-response gradient (TR #3 of the fleet trio). Emits `vector_mean`/`vector_m2` (N, 3) double + `scalar_mean`/`scalar_m2` (N,) double + `n_per_atom` (N,) uint64. Same gate as the TS pair (HasResult skip + mask=0). Minimum-viable v0 — no delta variants. Group is skipped entirely on `source_attached_count==0`. SDK: `AIMNet2ChargeResponseGradientWelfordGroup`. |
