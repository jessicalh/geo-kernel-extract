...
| ✓ | `MopacBondOrderWelfordTrajectoryResult` | Mopac | AV | per-bond Welford rollup of MOPAC Wiberg bond orders (`MopacResult.TopologyBondOrders()`, dimensionless). Bond axis B = `protein.BondCount()` parallel to `bonds.npy` from the TopologySidecar. Per-bond Welford state lives **inside the TR** as `std::vector<WelfordMoments> per_bond_` (BondLengthStats pattern) — no TrajectoryAtom field. Single channel, full canonical row: `order_{mean,std,m2,min,max,min_frame,max_frame}` (B,) + `n_per_bond` (B,) uint64. Same sparse-cadence + "absent, not faked" gate as TR5. MopacResult zeros out bonds it didn't report (NOT NaN), so per-bond `n_per_bond` equals `source_attached_count` (no per-bond skip). Group attr `bond_axis = "bonds.npy"` pins the axis convention for SDK consumers. TR6 of the 13-TR plan; clone of TR5 with bond axis. SDK: `MopacBondOrderWelfordGroup`. |
| ✓ | `MopacCoulombShieldingTimeSeriesTrajectoryResult` | MopacCoulomb | FO | per-atom (N, T, 5) double — five T2 components of the MOPAC Coulomb shielding contribution (`ConformationAtom.mopac_coulomb_shielding_contribution`). T2-only emission per the MopacCoulombResult.cpp:251 source comment "Pure T2 (EFG is traceless). gamma converts this to shielding" — the underlying EFG_total is symmetric traceless from the Poisson tracelessness projection at source, so T0+T1 are structurally zero. Attrs `irrep_layout="T2_m-2,T2_m-1,T2_m0,T2_m+1,T2_m+2"` / `normalization="isometric_real_sph"` / `parity="2e"` / `units="ppm"`. MopacCoulombResult attaches via `TimedAttach` (OperationRunner.cpp:183, NOT Require) — same sparse cadence + "absent, not faked" gate as TR5/TR6. WriteH5Group skips the entire `/trajectory/mopac_coulomb_shielding_time_series/` group when source never attached. TR7 of the 13-TR plan; combines TR4 T2-only emission with TR5 sparse-cadence gate. SDK: `MopacCoulombShieldingTimeSeriesGroup`. |
| ✓ | `MopacMcConnellShieldingTimeSeriesTrajectoryResult` | MopacMcConnell | FO | per-atom (N, T, **9**) double — all T0+T1+T2 components of the MOPAC-weighted McConnell bond-anisotropy shielding contribution (`ConformationAtom.mopac_mc_shielding_contribution`). **9-component emission** deliberately diverges from the 13-TR plan's "(N, T, 5)" entry — the source M_total accumulates the bond-anisotropy kernel `M_ab = 9·cos_θ·d̂_a·b̂_b - 3·b̂_a·b̂_b - (3·d̂_a·d̂_b - δ_ab)` which is **not symmetric-traceless** (T0 and T1 are nonzero in practice — Integration1P9J shows max\|T0\|≈0.96, max\|T1\|≈1.14, max\|T2\|≈0.98, comparable magnitudes). Per user direction 2026-05-21 "if not traceless write both" + the methods-accumulate principle. Attrs `irrep_layout="T0,T1_m-1,T1_m0,T1_m+1,T2_m-2,T2_m-1,T2_m0,T2_m+1,T2_m+2"` / `parity="0e+1o+2e"` (matches FF14SB McConnellShieldingTimeSeries convention) / `units="ppm"`. Same sparse-cadence gate as TR5/TR6/TR7. TR8 of the 13-TR plan. SDK: `MopacMcConnellShieldingTimeSeriesGroup`. |
| ✓ | `MopacVsFf14SbReconciliationTrajectoryResult` | MopacCoulomb + Coulomb | FO | per-atom-per-frame (N, T) double — `|cos(MOPAC_T2, FF14SB_T2)|` ∈ [0, 1] in the T2 5-vector subspace; cross-source comparison of Coulomb shielding tensors from MOPAC PM7+MOZYME Mulliken charges vs FF14SB-parameterised charges. **Cross-source gate**: requires BOTH `MopacCoulombResult` AND `CoulombResult` attached per frame; either-absent → all-N NaN cells + mask=0. **Per-atom NaN** when either-side |T2| < `near_zero_vector_norm_threshold` (cosine undefined at noise floor). WriteH5Group skips the entire `/trajectory/mopac_vs_ff14sb_reconciliation/` group when no frame had both attached. Attrs `parity="0e"` (rotation-invariant scalar) / `units="dimensionless"` / `zero_magnitude_threshold` (group attr with numeric value + units string). Integration1P9J shows mean\|cos\|≈0.85, range\|cos\|=[0.005, 1.000] — MOPAC vs FF14SB agree on tensor orientation ~85% on average but per-atom disagreement spans the full range (the calibration signal). TR9 of the 13-TR plan; new cross-source pattern, its own canonical (no clones). SDK: `MopacVsFf14SbReconciliationGroup`. |
