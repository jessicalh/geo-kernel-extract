...
| ✓ | `HydrationShellWelfordTrajectoryResult` | HydrationShell | AV | per-atom Welford rollup; 4 scalar channels with full delta variants. nearest_ion_distance Welford degrades — m2 NaN-from-n=1 (delta·(x−new_mean) = inf·NaN), mean NaN-from-n=2 (delta = inf−inf = NaN); `np.isfinite()` filter catches both regimes. Commit f15b33a. |
| ✓ | `HydrationGeometryTimeSeriesTrajectoryResult` | HydrationGeometry | FO | per-atom (N, T, 3)/(N, T) — Vec3 dipole_vector (e·Å units, not Debye) + Vec3 surface_normal + uint32 first_shell_count + 3 polarisation scalars (alignment / coherence / asymmetry). polarisation_signal_channels attr names the trio; reference_frame="SASA_normal". Source-attached gate on `HasResult<HydrationGeometryResult>()`. Commit 6931528 + R5 units fix. |
| ✓ | `HydrationGeometryWelfordTrajectoryResult` | HydrationGeometry | AV | per-atom Welford rollup; per-component Vec3 + 4 scalar Welford channels (asymmetry / alignment / coherence / shell_count) each with full delta variants. Source-attached gate skips Welford updates + invalidates prev_* on absent frames; mean_dt_ps + frame_index_range from attached-subset. Dipole units e·Å. Commit 6931528 + R5 units fix. |
