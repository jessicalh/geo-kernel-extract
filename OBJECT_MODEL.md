...
| ✓ | `AIMNet2PolarisabilityTimeSeriesTrajectoryResult` | AIMNet2Polarisability | FO | per-atom (N, T, 3) double `polarisability_vector` (e²/Å — gradient of L = Σ_j q_j² via torch autograd backward through AIMNet2 charge head) + (N, T) double `polarisability_scalar` (L2 norm, e²/Å). Both emitted per methods-accumulate. Attrs follow existing Vec3 TR convention: `irrep_layout_vector="x,y,z"`/`normalization_vector="cartesian"`/`parity_vector="1o"`; `irrep_layout_scalar="T0"`/`parity_scalar="0e"`. AIMNet2PolarisabilityResult RequireConformationResult'd alongside AIMNet2Result (always-attached); compute-time `conf.HasResult<AIMNet2PolarisabilityResult>()` gate emits NaN-placeholder + `source_attached_per_frame=0` on absent frames. |
| ✓ | `AIMNet2PolarisabilityWelfordTrajectoryResult` | AIMNet2Polarisability | AV | per-atom Welford rollup of the AIMNet2 polarisability gradient (TR #3 of the fleet trio). Emits `vector_mean`/`vector_m2` (N, 3) double + `scalar_mean`/`scalar_m2` (N,) double + `n_per_atom` (N,) uint64. Same gate as the TS pair (HasResult skip + mask=0). Minimum-viable v0 — no delta variants. Group is skipped entirely on `source_attached_count==0`. SDK: `AIMNet2PolarisabilityWelfordGroup`. |
