...
owned \code{Protein}, seats frame 0 as a permanent \code{MDFrameConformation},
allocates one \code{TrajectoryAtom} per protein atom, owns attached
\code{TrajectoryResult}s, and receives dense buffers at finalization.
...
Frame 0 is permanent and is an \code{MDFrameConformation}. Later streamed
frames are free-standing base \code{ProteinConformation} instances returned by
\code{TrajectoryProtein::TickConformation}; they point at the wrapped
\code{Protein} for topology and die at the end of the loop iteration. There is
no separate per-frame geometry class.
...
\code{OperationRunner} is the single in-tree home for the ordered
conformation-level sequence. It does not own state. It takes a mutable
\code{ProteinConformation} and a \code{RunOptions} value, computes result
objects, and attaches them.
...
\code{Trajectory::Run} is the trajectory orchestrator. It is the only code path
...
\item Read frame 0 and call \code{TrajectoryProtein::Seed}.
...
\item Run frame 0 through \code{OperationRunner}, dispatch trajectory results,
emit per-frame PDB/NPY, and record frame metadata.
...
The system's buffer discipline is constructor allocation plus scoped ownership.
Buffers are allocated at the object whose lifetime defines their validity.
...
\code{TrajectoryProtein::Seed} finalizes the wrapped protein, seats frame 0,
and then calls \code{InitTrajectoryAtoms}. That method clears and reserves the
vector to \code{protein\_->AtomCount()} and constructs one
...
\code{MDFrameConformation}. Current trajectory code seats only frame 0 through
\code{AddMDFrame}. Later frames use free-standing base
\code{ProteinConformation} objects from \code{TickConformation}.
