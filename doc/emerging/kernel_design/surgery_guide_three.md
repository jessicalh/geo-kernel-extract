...
Do these eight differently and the surgery is mostly **naming, switching on what's
already computed, and one careful sign pass** — not a rebuild.

## The eight cross-cutting rules (do differently)

1. **Emit the unit kernel; pin the physical scale separately — never fabricate a magnitude.** First
   classify each kernel **source-based** (ring, McConnell → unit geometric kernel + a scale that is
   literature/physics-pinned) vs **target-based** (EFG → *is* the physical field; no scale to ride).
...
4. **Per-Step altitude (the dragon).** The three emit tensor features, but they are not Step-1 inputs.
   Step 2 may use them only if ablation earns them. T2 stays sacred as DFT-angular evidence, **never**
   sold as a solution-shift predictor.
...
- **McConnell — already built.** Surgery is light: the deliberate re-bless, the two tidy-ups (DRY
  manifest, layout-string), the X–H ablation (Step 1 decides keep/drop vs DFT), and — optional, later —
...
**Ring pin-vs-learn → PINNED.** The producer emits the unit-current kernel. The physical scale is the
literature ring-current intensity in `σ = I·G`; do not learn a free kernel scale from this sample.
...
**Build requirement: every two-path consistency check ships as an ASSERT test, in the same
...
was DROPPED — consistency ≠ validation; we keep "HM-style," and
...
- **Geometric, source-based, cheaply upgradeable (the scale is pinned separately):** **McConnell** (unit `Q̂` →
