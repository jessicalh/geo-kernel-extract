...
*The aromatic ring-current effect on nuclear shielding, by two paths: Biot–Savart
(Johnson–Bovey) and an HM-style surface integral.*
...
Two classical models compute it by different mathematics:
...
- **HM-style surface integral.** A geometric model: a signed-area / surface integral over the ring, weighted by the
  dipolar `(3cos²θ−1)/r³` kernel. An entirely independent derivation of the *same* physical quantity.

Their agreement *by different math* is an internal **consistency/regression guard**, not a validation.
Where Johnson–Bovey and the HM-style surface integral concur on **sign, lobe structure, scalar shift, and
...
evidence**. The value is the near-ring sign/topology/orientation agreement, reported as **signed
...
unit kernel; pin the physical scale separately.** The literature **ring-current intensity** is the
...
physical scale is **pinned** from literature/physics, not learned as a free coefficient. The rediscover
layer carries a literature-scaled `jb_T*` via `LiteratureIntensity()`, separate from the unit-current
producer emit.

**The HM naming caveat (carry it honestly).** Our `HaighMallionResult` is a surface-integral
...
move is to call it **HM-style** unless a published-value benchmark or algebraic proof earns the literal
name. Self-correlation between our own paths does not earn it. Keep both `hm_H` (raw symmetric-traceless
surface kernel) and `hm_G` (the shielding tensor).
...
- **The intensity constants.** The current defaults read like older Giessner-Prettre / Pullman-style
  ...
  fitted factors, especially for His and the Trp five-membered ring. Treat the in-code constants as
  pinned literature/physics choices with source scatter to disclose, not as coefficients the sample learns.
...
- **The sign-and-units gauntlet across consumers.** Ring-current has **historical sign issues**, and the
  ...
  through**: the static catalog (`ppm_T_per_nA`), the BS H5 time-series (historically mislabelled `ppm`),
  the h5-reader viz (which multiplies `LiteratureIntensity()`), and any model-side scaled view.
  A unit-current kernel, a literature-scaled prediction, and a model input are **three
...
(it can be considered for Step 3 by ablation). The `2e` is DFT-angular evidence for Steps 1–2. T2 stays
sacred as that evidence, never sold as the solution-shift predictor; the solution-relevant ring signal is
the `0e`.

**The two-path, as a deliverable.** Today BS↔HM agreement exists as a regression guard
(`tests/test_batch_biot_savart_haigh_mallion.cpp` — per-ring + distance-binned convergence, the `T2`
cosine). It is **not emitted**, and it compares **absolute**
values. If the thesis is to *report* the two-path consistency, that comparison must become an emit carrying
...
4. **Switch on the dropped channels, separably.** Per-type `T1` (BS and HM emit per-type `T0`+`T2`, drop
   ...
   re-extract is cheap.
5. **The HM `T0` benchmark** (§2) — can earn the literal name only against published values/proof.
6. **The two-path emit** (above) — if we want to report the consistency diagnostic.
...
**Rule 1, restated:** unit kernel emitted; the literature intensity is the **pinned physical
scale**. Variants should be named by set (raw unit-current / `Case95-JB` / `Case95-HM`), never silently
mixed.

**Cross-kernel:** ring (magnetic) and charge/EFG (electric) **do not** overlap at the mechanism level —
they coexist; their `2e` parts merely *correlate* as geometric descriptors near aromatics. Any comparison
is a diagnostic, not a Step-1 input. The real magnetic overlap is **ring ↔ McConnell**,
handled by McConnell zeroing its aromatic category (not here).

**What this spec deliberately does NOT do:** learn final intensity constants from this sample;
assert HM-formula identity before a real benchmark; claim the tensor predicts the solution shift (the dragon
