...
> **Draft status.** Scaffold for the advisor meeting. Stage 1 numbers are stubbed pending the
> `learn/` material (recovering from archive); the Stage 3 section is stubbed pending
> `SPEC_LEARNING_MODEL`; the 1P9J interpolation figure is pending the scoping GO
> (`SPEC_INTERP_1P9J_SCOPING`). Markdown now for fast iteration → LaTeX/PDF for the desk once content locks.
