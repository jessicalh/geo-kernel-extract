...
- **Topology sidecar** (commits `f2781da`, `dc50917`, 2026-05-13).
  Per-protein `residues.npy` + `bonds.npy` + `rings.npy` +
  `ring_membership.npy` + `extraction_manifest.json` alongside an
  extended `atoms_category_info.npy` (+6 fields). `ArraySpec` gets
  `native_axis` / `irreps` / `units` / `sign_convention` /
  `tensor_rank` / `parity` / `mechanism` populated across all entries
  (resolves OI-016). SDK `load()` enforces axis-size + bond/ring
  reference invariants. Memory: `project_topology_sidecar_landed`.
